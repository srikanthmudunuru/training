...
//    //=> false
//
var isVowel = function (charector) {
  var type=typeof(charector);
  //console.log(type);
  var l=charector.length;
  if(type=="string")
  {
  if(l==1)
  {
    var x=charector.toUpperCase();
    ...
    throw " enter only single charector ";
  }
}
else {
...
function checkvowel(arg)
{
  arg=arg.toLowerCase();
  if("aeiou".indexOf(arg)!=-1)
  {
    return true;
  }
  else {
...
var isLowerCaseLetter = function (input) {
  ...
  if(type=="string");
  {
    if(input.length==1)
    {
      ...
      throw "enter only sinle charector";
    }
  }
  else {
    throw "enter strign type";
  }
...
//
var sumAToB = function (number1,number2) {
  if(typeof(number1)!="number"&&typeof(number2)!="number")
  {
     throw "inputs should be number";
...
var removeNonLetters = function (input) {
  {
    var res=input.replace(/[^A-Za-z0-9]/g,"");
    return res;
  }
